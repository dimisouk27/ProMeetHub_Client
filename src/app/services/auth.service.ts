...
import { ILoginForm, IRegisterForm } from '../models/AuthForms';
...
export class AuthService {
  ...
  register(registerForm: IRegisterForm): Observable<IAuth> {
    return this._httpClient
      ...
      .pipe(
        tap((auth) => this.connectedUser = auth)
      );
  }

  login(loginForm: ILoginForm): Observable<IAuth>{
    return  this._httpClient.post<IAuth>(`${this._BASE_URL}/login`, loginForm)
    .pipe(
      tap((auth) => this.connectedUser = auth)
      );
  }
